...
def L_festa(alpha, beta, gamma, sample_ratio):
    ...
    def loss(_, y_pred):
        ...
        random_idxs = tf.random.shuffle(tf.range((h-2)*(w-2)))[:int(h*w*sample_ratio)] #random_shuffle
        ...
        no_diag = tf.sparse_to_dense(ind_diag, [int(h*w*sample_ratio), h*w], K.repeat_elements(tf.constant([1.0]), int(h*w*sample_ratio), 0), validate_indices=False)*(tf.reduce_max(distance)+1) #sparse_to_dense
        ...
        mask = tf.sparse_to_dense(ind, [int(h*w*sample_ratio), h*w], K.repeat_elements(tf.constant([1.0]), int(h*w*sample_ratio)*8, 0), validate_indices=False) #sparse_to_dense
